...
```sql
SELECT yritys, MAX(palkka) AS korkein
FROM Tyontekijat GROUP BY yritys;
```
...
```sql
SELECT yritys, MAX(palkka) korkein
FROM Tyontekijat GROUP BY yritys;
```
...
```sql
SELECT yritys, COUNT(*) FROM Tyontekijat
GROUP BY yritys HAVING COUNT(*) >= 2;
```
...
```sql
SELECT yritys FROM Tyontekijat
GROUP BY yritys HAVING COUNT(*) >= 2;
```
...
Osa `ORDER BY projekti_id` järjestää tulostaulun rivit projektin id-numeron mukaan. Tässä tapauksessa projektit ovat 1 ja 3.
